        ...
        >
          {projects.map((item, index) => (
            <Grid item xs={2} sm={4} key={index}>
              <Item>
                <CustomProjectItem  item={item} index={index} />
              </Item>
            </Grid>
          ))}
        </Grid>
...
function CustomProjectItem({ item, index }) {
      ...
      <Box id="Front">
        <Card sx={{ borderRadius: 5, height: "100%", paddingBottom: 1}}>
          <CardHeader
            ...
            title={
              <Link color="#ffffff" href={item.link} underline={ item.link!=null ? "hover" : 'none'} target='_blank'>
